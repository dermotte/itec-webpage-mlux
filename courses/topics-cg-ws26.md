...
This course will dive into specific topics in the broad area of computer graphics. We will focus on (i) procedurally generated content (PCG). The course will require a significant amount of practical work, so programming knowledge is required. We will implement standard algorithms for PCG in computer games. The web page will be updated continuously. Grading is based on (i) mandatory assignments and (ii) a final project including a presentation, a live demo, and a written report.
